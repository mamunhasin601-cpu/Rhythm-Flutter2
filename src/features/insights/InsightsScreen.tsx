...
export default function InsightsScreen() {
  ...
  const exportCsv = () => {
    ...
    app.moods.forEach((m) =>
      rows.push(`mood;${m.date};${csvSafe(m.note ?? "")};${m.mood};${m.tags.join(",")}`)
    );
    app.focusSessions.forEach((s) =>
      rows.push(`focus;${s.date};${s.type};${s.focusMin};${s.completed ? "done" : "partial"}`)
    );
  ...
  const kpis: { icon: IconName; label: string; value: string; sub: string; tone: string }[] = [
    {
      ...
      sub: `${data.totalDone} за всё время`,
      tone: "text-aqua-300 bg-aqua-400/12",
    },
    {
      icon: "timer",
      label: "Фокус за неделю",
      value: fmtDur(data.weekFocus),
      sub: `${data.focusDelta >= 0 ? "+" : ""}${data.focusDelta}% к прошлой`,
      tone: "text-vio-300 bg-vio-400/12",
    },
    {
      icon: "play",
      label: "Flow-сессий",
      ...
      sub: "за 7 дней",
      tone: "text-ind-400 bg-ind-400/12",
    },
    {
      icon: "heart",
      label: "Среднее настроение",
      value: data.avgMood ? `${data.avgMood.toFixed(1)}/5` : "—",
      sub: `корреляция с делами ${data.r >= 0 ? "+" : ""}${data.r.toFixed(2)}`,
    ...
    <div id="insights" className="mx-auto max-w-[1060px] space-y-5">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="grid flex-1 grid-cols-2 gap-3 lg:grid-cols-4">
          {kpis.map((k, i) => (
            <section key={k.label} className={`anim-rise d-${i + 1} card p-4`}>
              ...
              <div className="mt-3 font-display text-[22px] font-bold leading-none tracking-tight text-mist-50">
                {k.value}
              </div>
              <div className="mt-1 text-[10.5px] font-extrabold uppercase tracking-wider text-mist-400">
                {k.label}
              </div>
        ...
        <div className="flex gap-2 print:hidden">
          <button className="btn btn-ghost !px-3 !py-2 !text-[12px]" onClick={exportCsv}>
            <I n="download" size={14} /> CSV
          </button>
          <button className="btn btn-ghost !px-3 !py-2 !text-[12px]" onClick={() => window.print()}>
            <I n="file" size={14} /> PDF
          </button>
        </div>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <section className="anim-rise d-2 card p-5 lg:col-span-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              <h3 className="font-display text-[15px] font-bold tracking-tight text-mist-50">
                Продуктивность: часы × дни недели
              </h3>
              <p className="mt-0.5 text-[12px] text-mist-400">
                Лучшие окна:{" "}
                {data.windows
                  .slice(0, 2)
                  .map((w) => `${minToHM(w.start)}–${minToHM(w.end)}`)
                  .join(" и ") || "копим данные"}
              </p>
            </div>
            <div className="flex items-center gap-2 text-[10px] font-bold text-mist-500">
              ...
              {[0.12, 0.3, 0.5, 0.75, 1].map((o) => (
                <span
                  ...
                  style={{ background: `rgba(55,214,192,${o})` }}
                />
              ))}
              больше
            </div>
          ...
          <div className="mt-4 overflow-x-auto">
            <div className="min-w-[560px]">
              ...
              >
                {HOURS.map((h) => (
                  <span key={h} className="text-center text-[9px] font-bold text-mist-500">
                    {h}
                  </span>
                ))}
              </div>
              {WD_SHORT.map((d, di) => (
                <div key={d} className="mt-[3px] flex items-center gap-2">
                  <span
                    className={`w-7 text-right text-[10px] font-extrabold ${di === weekdayIdx(todayKey()) ? "text-aqua-300" : "text-mist-500"}`}
                  >
                    {d}
                  </span>
                  <div
                        ...
                        style={{
                          background:
                            v === 0
                              ...
                              : `rgba(55,214,192,${0.15 + (v / data.heatMax) * 0.85})`,
                        }}
                      />
                    ))}
                  </div>
          ...
          <div className="mt-4 h-[180px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.cmp} margin={{ top: 5, right: 5, left: -22, bottom: 0 }} barGap={2}>
                ...
                <XAxis
                  dataKey="day"
                  tick={{ fill: "#67728c", fontSize: 10, fontWeight: 700 }}
                  tickLine={false}
                  axisLine={{ stroke: "rgba(255,255,255,0.08)" }}
                />
                <YAxis
                  tick={{ fill: "#67728c", fontSize: 10 }}
                  tickLine={false}
                  axisLine={false}
                  allowDecimals={false}
                />
                <Tooltip
                  ...
                  formatter={(v: number | string, n: string) => [v, n === "cur" ? "эта неделя" : "прошлая"]}
                />
        ...
        <section className="anim-rise d-4 card p-5">
          <h3 className="font-display text-[15px] font-bold tracking-tight text-mist-50">
            Фокус-часы по неделям
          </h3>
          <p className="mt-0.5 text-[12px] text-mist-400">
            из Flow Sessions · {fmtDur(data.weekFocus)} на этой
          </p>
          <div className="mt-4 h-[180px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.focus} margin={{ top: 5, right: 5, left: -18, bottom: 0 }}>
                ...
                <YAxis
                  ...
                  tickLine={false}
                  axisLine={false}
                  tickFormatter={(v: number) => `${Math.round(v / 60)}ч`}
                />
                <Tooltip
                  contentStyle={{
                    background: "#141927",
                  ...
                  formatter={(v: number | string) => [fmtDur(Number(v)), "фокус"]}
                />
                <Area
                  dataKey="min"
                  stroke="#37D6C0"
                  strokeWidth={2.2}
                  fill="rgba(55,214,192,0.13)"
                  type="monotone"
                />
              </AreaChart>
            </ResponsiveContainer>
        ...
        <section className="anim-rise d-5 card p-5 lg:col-span-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              <h3 className="font-display text-[15px] font-bold tracking-tight text-mist-50">
                Настроение ↔ выполненные задачи
              </h3>
              <p className="mt-0.5 text-[12px] text-mist-400">
                Каждая точка — день. r = {data.r >= 0 ? `+${data.r.toFixed(2)}` : data.r.toFixed(2)}
              </p>
            </div>
            <span
              className={`chip ${Math.abs(data.r) >= 0.4 ? "!border-aqua-400/30 !bg-aqua-400/10 !text-aqua-300" : ""}`}
            >
              {Math.abs(data.r) >= 0.6
                ? "сильная связь"
                : Math.abs(data.r) >= 0.3
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 5, right: 12, left: -14, bottom: 2 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number | string, n: string) => [v, n === "mood" ? "настроение" : "задач"]}
                />
