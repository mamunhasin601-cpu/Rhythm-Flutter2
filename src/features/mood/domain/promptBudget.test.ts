...
describe("getPromptWindow", () => {
  ...
  it("уважает кастомное время", () => {
    expect(getPromptWindow("morning", settings({ morningTime: 9 * 60 + 15 }))).toEqual({
      start: 555,
      end: 735,
    });
  });
...
describe("canShow — каждое ограничение", () => {
  ...
  it("1. выключен в настройках", () => {
    expect(canShow("morning", 9 * 60, epochAt(9 * 60), settings({ morningEnabled: false }), state())).toBe(
      false
    );
    expect(canShow("evening", 21 * 60, epochAt(21 * 60), settings({ eveningEnabled: false }), state())).toBe(
      false
    );
  });
  ...
  it("2. тихие часы (включая переход через полночь)", () => {
    ...
    expect(
      canShow(
        ...
        23 * 60 + 30,
        epochAt(23 * 60 + 30),
        settings({ quietStart: 22 * 60, quietEnd: 8 * 60 }),
        state()
      )
    ).toBe(false);
    ...
    expect(
      canShow(
        ...
        9 * 60,
        epochAt(9 * 60),
        settings({ quietStart: 8 * 60 + 30, quietEnd: 11 * 60 }),
        state()
      )
    ).toBe(false);
  });
  ...
  it("4. максимум 2 проактивных в день", () => {
    expect(
      canShow(
        ...
        epochAt(9 * 60),
        settings(),
        state({ proactiveShownToday: MAX_PROMPTS_PER_DAY })
      )
    ).toBe(false);
    expect(
      canShow(
        ...
        epochAt(9 * 60),
        settings(),
        state({ proactiveShownToday: MAX_PROMPTS_PER_DAY - 1 })
      )
    ).toBe(true);
  });
  ...
  it("6. один раз в день на тип", () => {
    expect(
      canShow("morning", 9 * 60, epochAt(9 * 60), settings(), state({ consumedTypesToday: ["morning"] }))
    ).toBe(false);
    // вечер при потраченном утре — ещё можно
    expect(
      canShow("evening", 21 * 60, epochAt(21 * 60), settings(), state({ consumedTypesToday: ["morning"] }))
    ).toBe(true);
  });
  ...
  it("7. недавний ручной check-in подавляет (только если включено)", () => {
    ...
    expect(
      canShow(
        ...
        9 * 60,
        now,
        settings({ skipIfRecentCheckin: false }),
        state({ recentManualCheckInAt: recent })
      )
    ).toBe(true);
    // старый check-in (5 ч) не подавляет
    expect(canShow("morning", 9 * 60, now, settings(), state({ recentManualCheckInAt: now - 5 * H }))).toBe(
      true
    );
  });
});
