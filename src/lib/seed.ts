...
const DAY_POOL: Tpl[] = [
  {
    title: "Deep Work: ключевой проект",
    icon: "briefcase",
    color: "violet",
    start: 540,
    dur: 90,
    energy: "high",
    tags: ["работа", "deep work"],
  },
  {
...
export function seedFor(user: User): {
  ...
  for (let back = 14; back >= 1; back--) {
    ...
    for (const tpl of shuffled) {
      ...
      tasks.push({
        ...
        tags: tpl.tags ?? [],
        energy: tpl.energy,
        status: statusFor(tpl.start, end, 24 * 60),
        source: "local",
        syncStatus: "synced",
        createdAt: date,
        updatedAt: date,
    ...
    moods.push({
      ...
      note:
        rnd() > 0.72
          ? ["День шёл по плану", "Много встреч, мало фокуса", "Хороший поток утром", "Устал к вечеру"][
              Math.floor(rnd() * 4)
            ]
          : undefined,
      source: "manual",
  ...
  const todayPlan: Tpl[] = [
    ...
    {
      title: "Планирование завтрашнего дня",
      icon: "target",
      color: "slate",
      start: 1260,
      dur: 20,
  ...
  for (const tpl of todayPlan) {
    ...
    tasks.push({
      ...
      tags: tpl.tags ?? [],
      energy: tpl.energy,
      status: statusFor(tpl.start, end, now),
      source: "local",
      syncStatus: "local",
      createdAt: today,
      updatedAt: today,
