...
export default function TaskModal({
  ...
  const smartTime = useMemo(() => bestTimeHint(app.tasks, { tags, energy }), [app.tasks, tags, energy]);
  ...
  useEffect(() => {
    ...
    if (task) {
      ...
      const s = draft
        ? clamp(snap(draft.startMin, 15), DAY_START, DAY_END - 60)
        : clamp(snap(new Date().getHours() * 60 + 30, 30), DAY_START, DAY_END - 60);
      setTitle("");
      setDescription("");
        ...
        <div className="space-y-4">
          {task?.source === "gcal" && (
            <div className="flex items-center gap-2.5 rounded-lg border border-ind-400/25 bg-ind-400/8 px-3 py-2 text-[12px] font-semibold text-ind-400">
              <I n="cloud" size={14} /> Импортировано из Google Calendar — изменения вернутся при
              синхронизации
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <Field label="Начало" error={errs.time}>
              <input
                ...
                value={minToHM(start)}
                onChange={(e) => e.target.value && setStartKeepDur(e.target.value)}
              />
            </Field>
            <Field label="Конец" hint={`${fmtDur(dur)}`}>
              <input
                ...
                value={minToHM(end)}
                onChange={(e) =>
                  e.target.value && setEnd(clamp(hmToMin(e.target.value), start + 15, DAY_END))
                }
              />
            </Field>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="mr-1 text-[11px] font-bold uppercase tracking-wider text-mist-500">
              Длительность
            </span>
            {DUR_CHIPS.map((d) => (
              <button
          ...
          <Field
            label="Повторение"
            hint={recurrence ? "Экземпляры на 7 дней вперёд появятся в таймлайне автоматически" : undefined}
          >
          ...
          <div className="flex items-center gap-2.5 border-t border-white/6 pt-4">
            ...
            {!task && (
              <button
                ...
                title="Сохранить как шаблон для быстрого создания"
              >
