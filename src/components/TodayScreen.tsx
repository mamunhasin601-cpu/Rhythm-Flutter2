...
import { selectNowTask } from "../features/timeline/nowTask";
...
export default function TodayScreen({
  ...
  useEffect(() => {
    ...
    if (!el) return;
    el.scrollTop =
      date === todayKey() ? Math.max(0, (nowMin() - DAY_START - 75) * PPM) : (9 * 60 - DAY_START) * PPM;
  }, [date]);
  ...
  const dayTasks = useMemo(
    () => app.tasks.filter((t) => t.date === date && !t.recurrenceRule),
    [app.tasks, date]
  );
  ...
  const energyPath = useMemo(() => {
    ...
    const w = 100;
    const xy = pts.map((p) => [((p.min - DAY_START) / (DAY_END - DAY_START)) * w, 100 - p.v]);
  ...
  return (
    <div className="mx-auto max-w-[1180px] space-y-5">
        ...
        <section className="anim-rise card flex min-w-0 flex-1 flex-col overflow-hidden">
          <header className="flex items-center gap-2 border-b border-white/6 px-4 py-3">
            ...
            <div className="min-w-0 text-center">
              <div className="font-display text-[14px] font-bold leading-tight text-mist-50">
                {relDayLabel(date)}
              </div>
          ...
          <div ref={scrollRef} className="relative flex-1 overflow-y-auto overscroll-contain">
            {total === 0 ? (
              <div className="p-5">
                ...
                >
                  <button
                    ...
                    onClick={() => onNewAt(date, clamp(snap(nowMin() + 30, 30), DAY_START, DAY_END - 60))}
                  >
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      app.toast(
                        added ? "success" : "error",
                        added
                          ? `Добавлено ${added} ${added === 1 ? "рутина" : "рутины"} из привычек`
                          : "Свободных слотов не нашлось"
                      );
                    }}
                  >
              ...
              <div className="flex select-none">
                ...
                {/* сетка */}
                <div
                  className="relative flex-1 cursor-crosshair border-l border-white/6"
                  style={{ height: H }}
                  onDoubleClick={dblCreate}
                >
                  {Array.from({ length: 18 }, (_, i) => DAY_START + i * 60).map((m) => (
                    <div
                      key={m}
                    ...
                    return (
                      <div
                        ...
                        className={`group absolute rounded-[9px] border transition-shadow ${
                          d
                            ? "z-30 cursor-grabbing shadow-2xl ring-1 ring-white/25"
                            : "z-10 cursor-grab hover:z-20 hover:shadow-xl"
                      ...
                      >
                        <div
                          className={`flex h-full flex-col overflow-hidden px-2.5 ${compact ? "justify-center" : "py-1.5"}`}
                        >
                          <div className="flex items-center gap-1.5">
                            ...
                            <span
                              className={`min-w-0 flex-1 truncate text-[12.5px] font-bold leading-tight text-mist-50 ${isDone ? "line-through" : ""}`}
                            >
                              {t.title}
                            </span>
                            {t.source === "gcal" && (
                            ...
                            {(t.parentTaskId || t.recurrenceRule) && (
                              <span
                                className="chip !px-1.5 !py-0 !text-[9px] !text-vio-300"
                              ...
                              {h > 64 && t.tags.length > 0 && (
                                <div className="mt-1 flex flex-wrap gap-1">
                                  {t.tags.slice(0, 2).map((tag) => (
                                    <span
                                      key={tag}
                                      className="rounded-md bg-white/5 px-1.5 py-[1px] text-[9.5px] font-bold text-mist-400"
                                    >
                                      #{tag}
                                    </span>
                                  ))}
                          ...
                          {isSkipped && !compact && (
                            <div className="mt-0.5 text-[10px] font-bold text-mist-500">
                              пропущено — день перестроен
                            </div>
                          )}
                        </div>
                        ...
                        {!isSkipped && (
                          <div
                            className={`absolute right-1.5 top-1.5 flex gap-1 transition-opacity ${d ? "opacity-0" : "opacity-0 group-hover:opacity-100"}`}
                          >
                            {!isDone ? (
                              <button
                            ...
                            <button
                              className={`flex h-6 items-center justify-center rounded-md border bg-ink-900/85 px-1 ${
                                armedDelete === t.id
                                  ...
                                  : "w-6 border-white/15 text-mist-300 hover:bg-white/10"
                              }`}
                  ...
                  {isToday && minute >= DAY_START && minute <= DAY_END && (
                    <div
                      ...
                      style={{ top: (minute - DAY_START) * PPM }}
                    >
      ...
      <SlotConflictDialog
        ...
        onPick={(slot) => {
          if (!conflict) return;
          const applied = app.updateTask(conflict.taskId, {
            startMin: slot.startMin,
            endMin: slot.startMin + conflict.dur,
          });
          if (applied) {
            app.toast(
              "info",
              `«${conflict.taskTitle}» → ${minToHM(applied.startMin)}–${minToHM(applied.endMin)}`
            );
          }
          setConflict(null);
        }}
...
function NowCard({
  ...
  const selection = selectNowTask(dayTasks, today, now);
  const current = selection?.kind === "current" ? selection.task : null;
  const overdue = selection?.kind === "overdue" ? selection.task : null;
  const next = selection?.kind === "next" ? selection.task : null;
  const done = dayTasks.filter((t) => t.status === "done").length;
  const total = dayTasks.length;
  ...
  if (current) {
    ...
    countdown = fmtLeft(Math.max(0, nowSec - overdue.endMin * 60));
  } else if (next) {
    countdown = fmtLeft(Math.max(0, next.startMin * 60 - nowSec));
  }

  const active = selection?.task ?? null;
  const colors = resolveColors(app.user);
      ...
      {active ? (
        <>
          <div className="mt-2 flex items-center gap-2">
            <span
              className="chip !border-transparent !px-1.5 !py-0.5 !text-[10px]"
              style={{ color: c, background: `${c}1c` }}
            >
              {current ? "идёт сейчас" : overdue ? "просрочено" : `в ${minToHM(next!.startMin)}`}
            </span>
            <span className="font-display text-[19px] font-bold tracking-tight text-mist-50 tabular-nums">
              {countdown}
            </span>
            <span className="text-[10.5px] font-bold text-mist-500">
              {current ? "до конца" : overdue ? "после срока" : "до начала"}
            </span>
          </div>
          <div className="mt-2 flex items-start gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[11.5px] font-semibold text-mist-400">
                {minToHM(active.startMin)}–{minToHM(active.endMin)} ·{" "}
                {fmtDur(active.endMin - active.startMin)}
              </div>
            </div>
          </div>
          {current && (
            <Bar
              className="mt-3"
              value={((now - current.startMin) / (current.endMin - current.startMin)) * 100}
              color={`linear-gradient(90deg,${c},#37D6C0)`}
            />
          )}
          <div className="mt-3.5 flex gap-2">
            ...
            {(current || overdue) && (
              <button
                ...
                title="Rhythm перестроит оставшийся день"
              >
                Пропустить
              </button>
            )}
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11.5px] text-mist-400">
              {done} из {total} задач — отличная работа
            </div>
          </div>
        </div>
      ) : (
  ...
  return (
    <div className="anim-rise d-2 card p-4">
      ...
      {log ? (
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block text-[10.5px] font-semibold text-mist-500">
              {minToHM(log.timeMin)} · отметить ещё
            </span>
          </span>
        ...
        <button
          className="btn btn-soft mt-2.5 w-full"
          onClick={() => app.openCheckIn()}
          data-testid="checkin-open"
        >
      ...
      {rs.length === 0 ? (
        ...
        <div className="mt-2.5 space-y-1.5">
          {rs.map((r) => (
            <div
              key={r.id}
              className="group flex items-center gap-2.5 rounded-lg border border-white/5 bg-white/[0.02] px-2.5 py-2 transition hover:bg-white/[0.045]"
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[10.5px] font-semibold text-mist-500">
                  ~{r.timeHint} · {fmtDur(r.durationMin)}
                </div>
              </div>
              <button
    ...
    <div className="anim-rise d-4 card relative overflow-hidden p-4">
      ...
      {slots[0] ? (
        <p className="mt-2.5 text-[12.5px] leading-relaxed text-mist-300">
          Пик энергии —{" "}
          <b className="font-display text-aqua-300">
            {minToHM(slots[0].start)}–{minToHM(slots[0].end)}
          </b>
          . Поставь сюда самую сложную задачу
          {rest[0] ? (
            <>
              {" "}
              , а в{" "}
              <b className="text-mist-100">
                {minToHM(rest[0].start)}–{minToHM(rest[0].end)}
              </b>{" "}
              — прогулку
            </>
          ) : (
        ...
        <p className="mt-2.5 text-[12.5px] text-mist-400">
          Сегодня ровный день без ярких пиков — чередуй фокус и отдых.
        </p>
      )}
      <p className="mt-2 text-[10.5px] font-semibold text-mist-500">
        На основе сна ({sleep.toFixed(1).replace(".0", "")} ч){mood ? " и чек-ина настроения" : ""}.
      </p>
    </div>
  );
}
