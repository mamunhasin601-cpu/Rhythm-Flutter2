...
export default function TogetherScreen() {
  ...
  const endSession = () => {
    ...
    app.toast(
      "success",
      `Sync-сессия завершена: ${min} ${plural(min, "минута", "минуты", "минут")} совместного фокуса. +${min * 5} XP`
    );
    setSession(null);
  };
      ...
      <div className="space-y-5">
          ...
          {!session ? (
            ...
            <div className="anim-rise mt-5">
              <div className="flex flex-wrap items-center gap-5">
                <div className="font-display text-[38px] font-bold leading-none tracking-tight text-mist-50 tabular-nums">
                  {fmt(elapsed)}
                </div>
                <div className="flex -space-x-2.5">
                  {session.joined.map((j, i) => (
                    <div
                      ...
                      style={{
                        background: `linear-gradient(120deg, ${FRIENDS[i % FRIENDS.length].color}, #6C7BFF)`,
              ...
              <div className="mt-4 flex items-center gap-2 text-[11.5px] font-semibold text-mist-500">
                ...
                {session.joined.length} {plural(session.joined.length, "участник", "участника", "участников")}{" "}
                · статусы скрыты, виден только фокус
              </div>
            </div>
          )}
        </section>
        ...
        <section className="anim-rise d-1 card p-5">
          ...
          <p className="mt-0.5 text-[12px] text-mist-400">
            Только статусы — без деталей расписания. Приватность прежде всего.
          </p>
          <div className="mt-4 space-y-2">
            {FRIENDS.map((f, i) => {
              ...
              return (
                <div
                  key={f.name}
                  className={`anim-rise d-${Math.min(i + 1, 6)} flex items-center gap-3 rounded-xl border border-white/5 bg-white/[0.02] px-3.5 py-2.5 transition hover:bg-white/[0.045]`}
                >
                  <div className="relative">
                    <div
                      className="flex h-9 w-9 items-center justify-center rounded-full text-[11px] font-extrabold text-white"
                      style={{ background: `linear-gradient(120deg, ${f.color}, #6C7BFF)` }}
                    >
                      {initials(f.name)}
                    </div>
                    <span
                      className={`absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-ink-850 ${meta.dot}`}
                    />
                  </div>
                  ...
                  {canInvite && (
                    <button
                      className="btn btn-aqua !px-2.5 !py-1 !text-[11px]"
                      onClick={() => app.toast("success", `Приглашение отправлено: ${f.name}`)}
                    >
      ...
      <aside className="space-y-4">
        ...
        <section className="anim-rise d-3 card p-5">
          ...
          <p className="mt-2 text-[12px] leading-relaxed text-mist-400">
            Друзья симулируются локально. В проде — Supabase Realtime: presence-статусы, общие блоки времени и
            live-сессии без опросов сервера.
          </p>
